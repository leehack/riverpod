...
import myProvider from "./providers/my_provider";
import multiProvider from "./providers/multi_providers";
...
<AutoSnippet language="dart" {...myProvider}></AutoSnippet>
...
<AutoSnippet language="dart" {...multiProvider}></AutoSnippet>
...
| [NotifierProvider]       | Returns any type                     | A filter condition / simple state object                             |
| [AsyncNotifierProvider]  | Returns a Future of any type         | A result from an API call                                            |
...
because of [issues with mutable state](/docs/concepts/why_immutability).
...
:::note
[code_generation] with `@riverpod` annotation generates `autoDispose` Provider.  
`@Riverpod(keepAlive: true)` annotation generates a non-disposable provider
...
[code_generation] generates the family provider when it takes parameters.

```dart
@riverpod
String myFamily(MyFamilyRef ref, int id) => '$id';
```
...
[code_generation]: /docs/about_code_generation
